      ...
      <div className="absolute left-0 mt-1 w-48 bg-white shadow-lg py-1 rounded-md">
        ...
        <div className="hidden md:flex items-center">
          <Link
            ...
            className="md:mr-6 px-6 py-2 text-sm font-semibold bg-primary text-white rounded-full hover:bg-maintext transition duration-200"
          >
            Book Now
          </Link>
        </div>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-40 bg-black bg-opacity-50 md:hidden" onClick={() => setSidebarOpen(false)}>
            ...
            <nav className="bg-white">
              {navItems.map((item) => {
                const isActive = pathname === item.href || item.options?.some((option) => option.href === pathname);
                
                return (
                
                <div key={item.title} className="mb-4 mt-10 justify-betweenflex items-center justify-between text-primary font-normal text-xs uppercase tracking-wider">
                    ...
                    >
                      <Link 
                          ...
                          className={`block text-primary font-normal uppercase ${
                            isActive ? 'border-b-2 pb-1 border-maintext' : ''
                          }`}
                        >
                        {item.title}
                      </Link>
                  ...
                  {item.hasDropdown && openDropdown === item.title && (
                    <ul className="ml-4 mt-2 space-y-2">
                      {item.options.map((option) => (
                        <li key={option.title} className="mt-1" onClick={() => setSidebarOpen(false)}>
                          <Link 
                              href={option.href} 
                              className={`text-xs m-1 text-maintext capitalize ${
                                pathname === option.href ? 'border-b-2 border-maintext' : ''
                              }`}
                            >
                            {option.title}
              ...
              <div className="mt-12 flex justify-center">
                <Button size="default" className="w-3/5 rounded-3xl">Book Now</Button>
              </div>
            </nav>
          </div>
        </div>
